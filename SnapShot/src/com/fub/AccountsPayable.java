...
public class AccountsPayable extends ImageGroup 
{
	...
	public void build() 
	{
		log("Building Accounts Payable...");
		System.out.println( "Building Accounts Payable...");
		String fileName = "";
      	...
      	try
		{
			...
			apmastStmt = connection.createStatement();
			countStmt = connection.createStatement();	
			countSet =   diimgStmt.executeQuery("SELECT count(*) AS ALLCOUNT from DATDUR.DIIMG WHERE DIOBJDAT >= '20100301' AND SUBSTRING(DIAIDX01V, (LENGTH(RTRIM(DIAIDX01V))), 1) = 'A'"); 
			while(countSet.next())
			{
				log( "Accounts Payable Count : " + countSet.getString("ALLCOUNT")); 
			}
			
			diimgSet = diimgStmt.executeQuery("SELECT * from DATDUR.DIIMG WHERE DIOBJDAT >= '20100301' AND SUBSTRING(DIAIDX01V, (LENGTH(RTRIM(DIAIDX01V))), 1) = 'A'"); 
			while (diimgSet.next())
			{	
				count++;
				...
				buffer.append("<VENDORNO>" +                diimgSet.getString("DIAIDX01V")+"</VENDORNO>");
				empty = true;
				mismatch = true;
				String index = diimgSet.getString("DIAIDX01V").trim(); 
				String val = index.substring(0, (index.length()-1));
				apmastSet = apmastStmt.executeQuery("Select (APCIF#) AS CIFNO from DATDUR.APMAST WHERE APACCT ='" + val  + "'" );
				outter:
				while (apmastSet.next())
				{
				  empty = false;
				  cfmastSet = cfmastStmt.executeQuery("Select * from DATDUR.CFMAST WHERE (CFCIF#) ='" + apmastSet.getString("CIFNO").trim()  + "'" ); 
				  while (cfmastSet.next())
				  {
					  ...
					  if( tid.length() < 9)
					  {
						  switch(cfmastSet.getString("CFSSNO").length())
						  {
				  ...
				  if(mismatch)
				  {
					 ...
					 mmbuffer.append("APMAST CIFNO;"     +  apmastSet.getString("CIFNO")    +"; ");
					 mmbuffer.append('\n');						
					 empty = true;	
					 write("C:\\FUBGlobal360_xmlexport\\no_match_log\\Master_No_Match_A.txt", mmbuffer.toString() );	
				     break outter;
				   }
				}
				if(empty)
				...
				if(count == 1)
				{
					File theDir = new File( "C:\\FUBGlobal360_xmlexport\\Type A\\set" + dirNum);
					theDir.mkdir();
					fileName = "C:\\FUBGlobal360_xmlexport\\Type A\\set" + dirNum + "\\" + diimgSet.getString("DIOBJNO") + ".xml";
				}
				else if( count < 50000)
				{
					fileName = "C:\\FUBGlobal360_xmlexport\\Type A\\set" + dirNum + "\\" + diimgSet.getString("DIOBJNO") + ".xml";
		...
		{
			log("building error AccountsPayable " + ex.toString());
			System.out.println( "building error  AccountsPayable " + ex.toString());
		}
		finally
        {
			cleanUp();	    	  
        }
		System.out.println("Build Complete Accounts Payable");
		log("Build Complete Accounts Payable");
	}
	...
	public void cleanUp()
    {
    	...
    	{
    		log("error closing connection AccountsPayable" + ex.toString());
    		System.out.println("error closing connection AccountsPayable " + ex.toString());
    	}
    }
