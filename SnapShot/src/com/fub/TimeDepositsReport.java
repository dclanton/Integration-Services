...
public class TimeDepositsReport extends ImageGroup 
{
	...
	public void build() 
	{
		...
		String fileName = "C:\\FUBGlobal360_xmlexport_03_16_2010\\Type_T_TID_0.txt";
	   	...
	   	try
		{
			...
			countSet =   diimgStmt.executeQuery("SELECT count(*) AS ALLCOUNT from DATDUR.DIIMG WHERE DIOBJDAT <= '20100228' AND SUBSTRING(DIAIDX01V, (LENGTH(RTRIM(DIAIDX01V))), 1) = 'T'");
			while(countSet.next())
			{
				log( "Time Deposits Count : " + countSet.getString("ALLCOUNT")); 
			}
			diimgSet =   diimgStmt.executeQuery("SELECT * from DATDUR.DIIMG WHERE DIOBJDAT <= '20100228' AND SUBSTRING(DIAIDX01V, (LENGTH(RTRIM(DIAIDX01V))), 1) = 'T'"); 
			while (diimgSet.next())
			{
				count++;
				...
				{
					empty = false;
					buffer.append("CDTYPE " +  cdmastSet.getString("TYPE")+", ");
