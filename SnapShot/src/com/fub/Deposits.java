...
public class Deposits extends ImageGroup  
{
	...
	public void build() 
	{
      	...
      	try
		{
			...
			countSet =  diimgStmt.executeQuery("SELECT count(*) AS ALLCOUNT from DATDUR.DIIMG WHERE DIOBJDAT >= '20100301' AND ( SUBSTRING(DIAIDX01V, (LENGTH(RTRIM(DIAIDX01V))), 1) = 'D' OR SUBSTRING(DIAIDX01V, (LENGTH(RTRIM(DIAIDX01V))), 1) = 'S' )");
			while(countSet.next())
			{
				log( "Deposits Count : " + countSet.getString("ALLCOUNT")); 
			}
			diimgSet =   diimgStmt.executeQuery("SELECT * from DATDUR.DIIMG WHERE DIOBJDAT >= '20100301' AND ( SUBSTRING(DIAIDX01V, (LENGTH(RTRIM(DIAIDX01V))), 1) = 'D' OR SUBSTRING(DIAIDX01V, (LENGTH(RTRIM(DIAIDX01V))), 1) = 'S') "); 
			while (diimgSet.next()  )
			{
				count++;
				...
				{
					empty = false;
					buffer.append("<SCCODE>" +  ddmastSet.getString("SCCODE")+"</SCCODE>");
				...
				if(count == 1)
				{
					File theDir = new File( "C:\\FUBGlobal360_xmlexport\\Type D or S\\set" + dirNum);
					theDir.mkdir();
					fileName = "C:\\FUBGlobal360_xmlexport\\Type D or S\\set" + dirNum + "\\" + diimgSet.getString("DIOBJNO") + ".xml";
				}
				else if( count < 50000)
				{
					fileName = "C:\\FUBGlobal360_xmlexport\\Type D or S\\set" + dirNum + "\\" + diimgSet.getString("DIOBJNO") + ".xml";
